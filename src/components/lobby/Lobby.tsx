...
import { useMatchActions, usePublicMatches } from '@/data';
...
export function Lobby() {
  ...
  const { matches: publicMatches, isLoading: isLoadingPublicMatches } = usePublicMatches();
  const { createMatch, joinMatch, fetchMatchByJoinCode } = useMatchActions();
  ...
  const handleCreateGame = async () => {
    ...
    createMatch({ playerId: user.uid, isPublic })
      .then(({ matchId }) => {
  ...
  const handleJoinWithCode = async () => {
    ...
    setError(null);
    try {
        const match = await fetchMatchByJoinCode(joinCode);
  ...
  const handleJoinMatch = async (matchId: string) => {
    if (!user) return;
    setIsLoading(true);
    joinMatch({ matchId, playerId: user.uid })
      .then(() => {
        router.push(`/game/${matchId}`);
      })
      .catch(() => {
